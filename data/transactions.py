...
def _generate_legitimate_transactions(
    ...
    for i in range(total_count):
        ...
        amount = round(
            rng.uniform(50, expected["expected_monthly_credits"] / monthly_count * 2), 2
        )

        is_credit = rng.random() < 0.5

        desc = rng.choice(LEGITIMATE_DESCRIPTIONS).replace(
        ...
        txns.append(
            {
                ...
                "amount": amount,
                "currency": "USD",
                "counterparty": _random_counterparty(
                    rng, all_account_ids, account_id, external=True
                ),
                "description": desc,
...
def _get_patterns_for_account(idx: int, rng: random.Random) -> List[str]:
    ...
    pattern_map = {
        ...
        2: ["structuring"],
        3: ["rapid_movement"],
        4: ["structuring", "rapid_movement"],
        5: ["round_tripping"],
        6: ["layering"],
        7: ["layering", "shell_company"],
        8: ["network_structuring", "shell_company"],
    }
...
def _generate_pattern(
    ...
    if pattern_type == "structuring":
        ...
        return _pattern_rapid_movement(
            rng, account_id, profile, all_account_ids, days, base_date
        )

    elif pattern_type == "round_tripping":

        return _pattern_round_tripping(
            rng, account_id, profile, all_account_ids, days, base_date
        )

    elif pattern_type == "layering":

        return _pattern_layering(
            rng, account_id, profile, all_account_ids, days, base_date
        )
        ...
        return _pattern_network_structuring(
            rng, account_id, profile, all_account_ids, days, base_date
        )
...
def _pattern_structuring(
    ...
    for i in range(num_deposits):
        ...
        amount = round(rng.uniform(8200, 9900), 2)

        txns.append(
            {
                "transaction_id": f"TXN-{account_id}-S{i + 1:04d}",
                "account_id": account_id,
                "date": (window_start + timedelta(days=day_offset)).strftime(
                    "%Y-%m-%d"
                ),
...
def _pattern_layering(
    ...
    for i in range(num_layers):
        ...
        for j in range(num_splits):
            ...
            split_amount = round(
                remaining / (num_splits - j) * rng.uniform(0.8, 1.2), 2
            )
...
def _pattern_network_structuring(
    ...
    for i in range(num_inflows):
        ...
        amount = round(rng.uniform(7500, 9800), 2)
        ...
        txns.append(
            {
                ...
                "counterparty": rng.choice(
                    [aid for aid in all_ids if aid != account_id]
                ),
...
def _random_counterparty(
    rng: random.Random, all_ids: List[str], exclude_id: str, external: bool = False
) -> str:
