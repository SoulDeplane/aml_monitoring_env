...
def generate_medium_scenario(rng: random.Random) -> Dict[str, Any]:
    ...
    alerts = [
        _make_alert("AML-M001", "ACC-0001", "structuring",
                     "Multiple sub-threshold cash deposits totaling $72,450 in 14 days.",
                     0.82, True),
        _make_alert("AML-M002", "ACC-0003", "rapid_movement",
                     "Large wire transfers received and immediately forwarded. Funds held for less than 48 hours.",
                     0.76, True),
        _make_alert("AML-M003", "ACC-0005", "round_tripping",
                     "Suspicious circular fund flows detected. $180,000 sent to offshore account returned through different entity.",
                     0.88, True),
        _make_alert("AML-M004", "ACC-0010", "velocity",
                     "Unusual spike in wire transfer activity — 15 transfers in 3 days vs normal 2/month.",
                     0.52, False),  # Legitimate seasonal business
        _make_alert("AML-M005", "ACC-0012", "geographic_risk",
                     "Multiple transactions involving high-risk jurisdiction (Cayman Islands).",
                     0.48, False),  # Legitimate international business
    ]

    expected = {
        "ACC-0001": {"suspicious": True, "patterns": ["structuring"], "action": "flag"},
        "ACC-0003": {"suspicious": True, "patterns": ["rapid_movement"], "action": "flag"},
        "ACC-0005": {"suspicious": True, "patterns": ["round_tripping"], "action": "file_sar"},
...
def generate_hard_scenario(rng: random.Random) -> Dict[str, Any]:
    ...
    review_accounts = [
        "ACC-0004", "ACC-0006", "ACC-0007", "ACC-0008",
        "ACC-0011", "ACC-0013", "ACC-0015", "ACC-0018",
    ]

    alerts = [
        _make_alert("AML-H001", "ACC-0004", "combined_patterns",
                     ...
                     0.91, True),
        _make_alert("AML-H002", "ACC-0006", "layering",
                     ...
                     0.87, True),
        _make_alert("AML-H003", "ACC-0007", "shell_company",
                     "Multiple large transfers to/from entities registered in offshore jurisdictions with no clear business purpose.",
                     0.93, True),
        _make_alert("AML-H004", "ACC-0008", "network_activity",
                     "Account receiving coordinated sub-threshold transfers from 12 linked accounts, subsequently wired to offshore entity.",
                     0.89, True),
        _make_alert("AML-H005", "ACC-0011", "geographic_risk",
                     "Wire transfers to multiple high-risk jurisdictions totaling $340,000 in 90 days.",
                     0.58, False),  # Legitimate import/export
        _make_alert("AML-H006", "ACC-0013", "velocity",
                     "Transaction count 300% above historical average.",
                     0.42, False),  # Seasonal spike — legitimate
        _make_alert("AML-H007", "ACC-0015", "large_cash",
                     "Series of cash deposits totaling $85,000 in 30 days.",
                     0.55, False),  # Legitimate cash-heavy business (restaurant)
        _make_alert("AML-H008", "ACC-0018", "layering",
                     "International wire received, split into 3 domestic transfers, then consolidated and wired to BVI entity.",
                     0.84, True),
    ]

    expected = {
        "ACC-0004": {"suspicious": True, "patterns": ["structuring", "rapid_movement"], "action": "file_sar"},
        "ACC-0006": {"suspicious": True, "patterns": ["layering"], "action": "file_sar"},
        "ACC-0007": {"suspicious": True, "patterns": ["layering", "shell_company"], "action": "file_sar"},
        "ACC-0008": {"suspicious": True, "patterns": ["network_structuring", "shell_company"], "action": "file_sar"},
        ...
        "ACC-0018": {"suspicious": True, "patterns": ["layering"], "action": "file_sar"},
    }

    # Network relationships (for get_network tool)
    network = {
        "ACC-0006": {
            "connected_accounts": ["ACC-0007", "ACC-0008"],
            "shared_counterparties": ["Vanguard Offshore Holdings Ltd", "Neptune Ventures BVI"],
            "relationship": "Accounts share counterparties and transfer patterns suggest coordinated activity",
        },
        "ACC-0007": {
            "connected_accounts": ["ACC-0006"],
            "shared_counterparties": ["Cascade International Trust", "Vanguard Offshore Holdings Ltd"],
