...
class AMLMonitoringEnvironment(MCPEnvironment):
    ...
    def __init__(self) -> None:
        ...
        def review_transactions(
            ...
            return env_ref._handle_review_transactions(
                account_id, days, min_amount, transaction_type
            )
        ...
        def file_sar(
            ...
            return env_ref._handle_file_sar(
                account_id, patterns, narrative, estimated_amount
            )
    ...
    def reset(
        ...
        self._last_feedback = (
            "AML monitoring session started. Review the pending alerts."
        )
    ...
    def step(self, action, timeout_s=None, **kwargs) -> Observation:
        ...
        if self._state.step_count >= max_steps:
            ...
            return self._make_observation(
                reward=-0.1, feedback=f"Step limit ({max_steps}) reached."
            )
    ...
    def _handle_review_transactions(
        self, account_id: str, days: int, min_amount: float, transaction_type: str
    ) -> str:
        ...
        self._repeated_actions[action_key] = (
            self._repeated_actions.get(action_key, 0) + 1
        )
        ...
        all_txns = self._scenario.get(
            "all_transactions", self._scenario.get("transactions", {})
        )
        ...
        for t in txns[:30]:
            ...
            clean = {
                k: v
                for k, v in t.items()
                if k not in ("is_suspicious_ground_truth", "pattern_type")
            }
        ...
        return json.dumps(
            {
                ...
                "summary": {
                    ...
                    "cash_deposit_total": round(
                        sum(t["amount"] for t in cash_deposits), 2
                    ),
    ...
    def _handle_check_profile(self, account_id: str) -> str:
        ...
        profiles = self._scenario.get(
            "all_profiles", self._scenario.get("profiles", {})
        )
    ...
    def _handle_analyze_pattern(self, account_id: str, pattern_type: str) -> str:
        ...
        if pattern_type not in valid_patterns:
            ...
            return json.dumps(
                {"error": f"Invalid pattern_type. Valid: {valid_patterns}"}
            )
        ...
        if pattern_type == "structuring":
            ...
            if len(cash_deps) >= 3:
                ...
                indicators.append(
                    f"Found {len(cash_deps)} cash deposits between $7,500-$9,999"
                )
            ...
            wire_in = [
                t
                for t in txns
                if t.get("type") in ("wire_transfer", "foreign_wire")
            ...
            if wire_in and wire_out:
                ...
                indicators.append(
                    f"Found {len(wire_in)} incoming and {len(wire_out)} outgoing wire transfers"
                )

                indicators.append(
                    "Check timing between deposits and withdrawals for pass-through pattern"
                )
            ...
            offshore_out = [
                ...
                and t.get("country_destination") not in ("United States", "")
            ]
            ...
            offshore_in = [
                ...
                and t.get("country_origin") not in ("United States", "")
            ]

            if offshore_out and offshore_in:

                indicators.append(
                    f"Found {len(offshore_out)} outgoing and {len(offshore_in)} incoming international transfers"
                )

                indicators.append(
                    "Check if funds sent out return through different entities"
                )

        elif pattern_type == "layering":

            multi_hop = [
                t for t in txns if t.get("type") in ("wire_transfer", "foreign_wire")
            ]
            ...
            if len(unique_counterparties) >= 4:
                ...
                indicators.append(
                    f"Transactions involve {len(unique_counterparties)} distinct counterparties"
                )

                indicators.append(
                    "Multiple jurisdictions and entities suggest layering"
                )
            ...
            shell_txns = [
                t for t in txns if t.get("counterparty") in SHELL_COMPANY_NAMES
            ]
            ...
            if shell_txns:
                ...
                indicators.append(
                    f"Found {len(shell_txns)} transactions with known offshore entities: {', '.join(shells)}"
                )
            ...
            if len(sub_threshold) >= 3:
                ...
                indicators.append(
                    f"Received {len(sub_threshold)} sub-threshold transfers from {len(senders)} accounts"
                )
    ...
    def _handle_get_network(self, account_id: str) -> str:
        ...
        for cp in counterparties:
            ...
            counterparties[cp]["total_amount"] = round(
                counterparties[cp]["total_amount"], 2
            )
        ...
        if account_network:
            ...
            result["connected_internal_accounts"] = account_network.get(
                "connected_accounts", []
            )

            result["shared_counterparties"] = account_network.get(
        ...
        self._last_feedback = (
            f"Network map for {account_id}: {len(counterparties)} counterparties found."
        )

        return json.dumps(result)

    def _handle_flag(
        self, account_id: str, patterns: list, evidence: str, risk_assessment: str
    ) -> str:
        ...
        if is_correct:
            ...
            feedback = (
                f"Account {account_id} flagged, but this may be a false positive."
            )
        ...
        return json.dumps(
            {"flagged": True, "account_id": account_id, "feedback": feedback}
        )

    def _handle_file_sar(
        self, account_id: str, patterns: list, narrative: str, estimated_amount: float
    ) -> str:
        ...
        if should_sar:
            ...
            feedback = (
                f"SAR filed for {account_id}. Correct — this account requires a SAR."
            )
        ...
        return json.dumps(
            {"sar_filed": True, "account_id": account_id, "feedback": feedback}
        )
    ...
    def _handle_clear(self, account_id: str, justification: str) -> str:
        ...
        if should_clear:
            ...
            feedback = (
                f"Alert for {account_id} cleared. Correct — this was a false positive."
            )
        ...
        return json.dumps(
            {"cleared": True, "account_id": account_id, "feedback": feedback}
        )
