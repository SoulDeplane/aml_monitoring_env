...
def main():
    ...
    avg = sum(r.get("overall_score", 0.0) for r in results.values()) / max(len(results), 1)
    print(f"  {'AVERAGE':8s}: {avg:.4f}")
    print(f"  Total time: {total_time:.1f}s")

    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "inference_results.json")
